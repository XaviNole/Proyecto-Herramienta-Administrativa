...
export class AddDoctorComponent {
  ...
  save(){
    ...
    // if(this.hours_selecteds.lenght == 0){
    ...
    this.doctorsService.registerDoctors(formData).subscribe((resp:any)=>{
      console.log(resp);
      if(resp.message==403){
    ...
    if(INDEX != -1){
      this.hours_selecteds.splice(INDEX,1);  
        ...
        this.hours_selecteds.push({
          "day": day, 
          "day_name": day.day, 
          "hour_day":hours_days, // Eso de arriba hace que cada valor pertenezca a su grupo
          "hour":hours_days.hour,
          "grupo": "none",
          "item":item,
        });
    }
  ...
  addHourAll(hours_days:any,day:any){
    let INDEX = this.hours_selecteds.findIndex((hour:any)=> hour.day_name == day.day 
                                              && hour.hour == hours_days.hour && hour.grupo == "all");

    let COUNT_SELECTED = this.hours_selecteds.filter((hour:any)=> hour.day_name == day.day 
                                              && hour.hour == hours_days.hour).length;
    if(INDEX != -1 && COUNT_SELECTED == hours_days.items.length){
      hours_days.items.forEach((item:any) =>{
        let INDEX = this.hours_selecteds.findIndex((hour:any)=> hour.day_name == day.day 
                                              && hour.hour == hours_days.hour
                                              && hour.item.hour_start == item.hour_start && hour.item.hour_end == item.hour_end);
        if(INDEX != -1){
  ...
  addHourAllDay($event:any,hours_days:any){

    let INDEX = this.hours_selecteds.findIndex((hour:any)=> hour.hour == hours_days.hour );
    ...
    if(INDEX != -1 && !$event.currentTarget.checked){
      ...
      setTimeout(() => {
        this.days_week.forEach((day)=>{
          this.addHourAll(hours_days,day);
        })
      }, 25);
    }
  ...
  isCheckHourAll(hours_days:any,day:any){
    let INDEX = this.hours_selecteds.findIndex((hour:any)=> hour.day_name == day.day 
                                              && hour.hour == hours_days.hour && hour.grupo == "all");
  ...
  isCheckHour(hours_days:any,day:any,item:any){
    let INDEX = this.hours_selecteds.findIndex((hour:any)=> hour.day_name == day.day 
                                              && hour.hour == hours_days.hour
                                              && hour.item.hour_start == item.hour_start && hour.item.hour_end == item.hour_end);
    if(INDEX != -1){
